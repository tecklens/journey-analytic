...
export class JaSdk {
    ...
    async getConfig() {
        try {
            ...
            if (this.websiteInfo) {
                ...
                this.websiteId = response.websiteId
                this.enableReplay = response.enableReplay
            }
        } catch (e) {
            this.log('err fetch config', e)
        }
    }
    ...
    async syncRecording() {
        this.intervalId = setInterval(async () => {
            if (this.sessionId) {
                const rrEvents = await this.storageRREvent?.getAllEvents();
                const events = await this.storageEvent?.getAllEvents();
                // console.log(events)

                if ((events && events.length > 0) || (rrEvents && rrEvents.length > 0)) {
                    let attempts = 0;
                    let success = false;
                    while (attempts < 2 && !success) {
                        try {
                            ...
                            if (response.ok) {
                                ...
                                await this.storageEvent?.clearAllEvents()
                                success = true;
                                break;
                            } else {
    ...
    addEvent(key: string, data: Record<string, any>) {
        ...
        if (this.sessionId) {
            ...
            this.storageEvent?.addEvent(payload).then(() => {
