...
export function ReplaySettingForm() {
    ...
    useEffect(() => {
        if (project.data) {
            form.reset({
                status: project.data?.status == 1,
                users: project.data?.users?.map((e: string, idx: number) => ({id: idx, text: e}))
            })
        }
    }, [project.status])
                ...
                <FormField
                    ...
                    render={({field}) => (
                        <FormItem className="flex flex-col items-start">
                            ...
                            <FormDescription>
                                Note: when recording user sessions, your website will load additional libraries to
                                record sessions and
